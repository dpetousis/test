...
input double const f_percWarp = 0.25;
input double const f_percTP = 0.35;
int const bollinger_delay = 5; //should be in input file
input double m_profitInAccCcy=1;
double m_filter[4] ={30,7,15,60};      // bollinger freq,fast filter freq <2,fast filter freq, fast filter freq>£200
...
double const f_lossLevelAdjust = 200;	// absolute level of loss after which we adjust
double const f_percTPAdjust = 0.26;		// kicks in after the loss level above is reached
int const i_bandsHistoryAdjust = 50;
...
// INDICATOR BUFFERS ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		if ((m_sequence[0]<0) || (m_sequence[2]<1.5 && iBarShift(m_names,timeFrame,m_lastTicketOpenTime,false)==0)) {	// no sequence or new sequence opened this bar                      // SF
         ...
         f_fastFilterPrev = iMA(m_names,0,(int)m_filter[1],0,3,PRICE_CLOSE,2);
			...
			else if (m_sequence[2]>1.5 && m_sequence[1]<-f_lossLevelAdjust) { fFilterCorrect = (int)m_filter[3]; }  // Third filter value when long sequence
         ...
         if (fFilterCurrent!=fFilterCorrect) {
				...
				//temp_oldFF = iCustom(m_names,0,"petousis_decycler",m_filter[1],filter_history,1,1); 
				//temp_newFF = iCustom(m_names,0,"petousis_decycler",m_filter[2],filter_history,1,1);
				...
				if (b_transition) { //transition if current slow FF is in the same region as current fast FF
					...
					//f_fastFilterPrev = iCustom(m_names,0,"petousis_decycler",m_filter[2],filter_history,1,2);
				...
				//f_fastFilter = iCustom(m_names,0,"petousis_decycler",m_sequence[6],filter_history,1,1);   
				//f_fastFilterPrev = iCustom(m_names,0,"petousis_decycler",m_sequence[6],filter_history,1,2);
          ...
          if (temp_T[0] < 0 || temp_T[1] < 0) {
            if (m_sequence[0]<0) {         // either new sequence or manually set (-1) mid-sequence, so update the pips
               ...
               if (m_sequence[1]<-f_lossLevelAdjust) {			// adjust if needed
                  m_bandsTSAvg=0.0;
                  m_barTSAvg=0.0;
                  statistics(m_bandsTSAvg,m_barTSAvg,50);
                  m_bollingerDeviationInPips = NormalizeDouble(f_percAvBandSeparation*m_bandsTSAvg,0);
               ...
               if (f_fastFilter>f_bollingerBand+m_slowFilterGap) { 
                  ...
                  if (MathAbs(m_sequence[0]-f_fastFilter)>f_adjustLevel*m_bollingerDeviationInPips*MarketInfo(m_names,MODE_POINT)) { 
					...
					else if (m_sequence[1]<-f_lossLevelAdjust && !b_slowFilterAdjusted) {
						m_sequence[0] = MathMin(m_sequence[0],f_fastFilter - 0.5 * m_bollingerDeviationInPips*MarketInfo(m_names,MODE_POINT));
                  ...
                  if (MathAbs(m_sequence[0]-f_fastFilter)>f_adjustLevel*m_bollingerDeviationInPips*MarketInfo(m_names,MODE_POINT)) { 
						...
						m_sequence[0] = MathMax(m_sequence[0],f_fastFilter + 0.5*m_bollingerDeviationInPips*MarketInfo(m_names,MODE_POINT));
						b_slowFilterAdjusted = true;
					}
   		         //else { m_slowFilter = m_sequence[0]; }
               }
