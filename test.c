...
int OnInit()
  {
   ...
   string arr_sequence[], arr_ticket[];
   ...
   for(int i=0; i<i_namesNumber; i++) {
      ...
      if (b_readLastWeekDataFromTradePopulation) {
         m_sequence[i][0] = -1.0;      //Initialize to -1,0,1 
         m_sequence[i][2] = 1.0;  
...
//---
   ...
   string const s_sequenceFileName = StringConcatenate("sequence_",IntegerToString(i_stratMagicNumber),".txt");
   string const s_ticketFileName = StringConcatenate("ticket_",IntegerToString(i_stratMagicNumber),".txt");
   writeOutputToFile(s_sequenceFileName,s_ticketFileName);
...
// VARIABLE DECLARATIONS /////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   double
   temp_vwap=-1.0,
   SL,TP,BID,ASK,f_fastFilterPrev=0.0,f_central=0.0,f_loss=0,f_lossComment=0,f_pnlSequence=0,f_pnlSingle=0,
   ...
   if (m_tradeFlag[i]==true) {
      for(int k=0; k<m_tradesNumber[i]; k++) {
	   ...
	   if (!b_multiPositionOpen && i_ticketSum>0) {    // order is closed if no open positions but ticket numbers not yet reset
		   ...
		   else { b_sequenceCriterion = (f_pnlSingle<=-0.01); }
...
if ((int)GlobalVariableGet("gv_createSnapshotForStrategy") == i_stratMagicNumber) {			// only enter loop if 
   ...
   res = writeOutputToFile(s_sequenceFileName,s_ticketFileName);
...
// CLOSING ORDERS //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 // OPENING ORDERS //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      ...
      for(int k=0; k<m_tradesNumber[i]; k++) {
         if (m_isPositionPending[i][k]==true && m_open[i][k]==0) {     // if pending order exists -> modify pending order
         		...
         		if (m_positionDirection[i][k]==1) { 
					...
					if (b_trendstalker) { SL=NormalizeDouble(ASK - m_bollingerDeviationInPips[i]*MarketInfo(m_names[i],MODE_POINT),(int)MarketInfo(m_names[i],MODE_DIGITS)); }    // Calculating SL of opened
					else { SL=NormalizeDouble(ASK - f_percVolstalkerSL * m_bollingerDeviationInPips[i]*MarketInfo(m_names[i],MODE_POINT),(int)MarketInfo(m_names[i],MODE_DIGITS)); }
                  TP=NormalizeDouble(ASK + m_bollingerDeviationInPips[i]*MarketInfo(m_names[i],MODE_POINT),(int)MarketInfo(m_names[i],MODE_DIGITS));   // Calculating TP of opened
            		...
            		if (b_trendstalker) { SL=NormalizeDouble(BID + m_bollingerDeviationInPips[i]*MarketInfo(m_names[i],MODE_POINT),(int)MarketInfo(m_names[i],MODE_DIGITS)); }    // Calculating SL of opened
            		else { SL=NormalizeDouble(BID + f_percVolstalkerSL*m_bollingerDeviationInPips[i]*MarketInfo(m_names[i],MODE_POINT),(int)MarketInfo(m_names[i],MODE_DIGITS)); }
      ...
      if (!(b_noNewSequence && m_sequence[i][0]<0)) {  // Send order when no new sequence flag is off 
         for(int k=0; k<m_tradesNumber[i]; k++) {
            ...
            if (m_open[i][k]<0 || m_open[i][k]>0) {
               if ((-m_sequence[i][1]<f_creditPenaltyThreshold && f_creditBalance>0) && m_tradesNumber[i]==1 && b_trendstalker) { // apply penalty
               ...
               if (MathAbs(f_loss)>m_profitInUSD[i]*f_percWarp) {
      	    	   m_lots[i] = NormalizeDouble(MathMax(m_lotMin[i],(MathAbs(f_loss)/f_percWarp) / m_accountCcyFactors[i] / m_bollingerDeviationInPips[i]),m_lotDigits[i]);
         	    }
         	    else {
            ...
            if (m_open[i][k]>0) {                                       // criterion for opening Buy
               ...
               s_comment = StringConcatenate(IntegerToString(m_myMagicNumber[i]),s_tradeFlag,"_",DoubleToStr(temp_vwap,(int)MarketInfo(m_names[i],MODE_DIGITS)),s_adjFlag,"_",DoubleToStr(-f_lossComment,2),"_",DoubleToStr(m_sequence[i][2],0));
  ...
  bool writeOutputToFile(string sequenceFileName, string ticketFileName)
  {
