...
#define TRADESPERNAMEMAX 4

// INPUTS ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//input switches & constants
extern bool b_noNewSequence = false;	// extern: so can be changed during program execution
...
input string const s_symbolAppendix = "";	// algo, pro or none usually
...
int i_namesNumber=0,f_ordersTotal = 0;
bool b_accountingWriteDone=false,b_pendingGlobal=false;
...
string const m_commentTradeFlag[TRADESPERNAMEMAX] = {"A","B","C","D"};
...
// VARIABLE DECLARATIONS /////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   uint tick=0;
   bool b_transition=false,b_tradeClosedBySL=false,res,b_pending=false,b_multiPositionOpen=false;
   ...
   double temp_pnl[TRADESPERNAMEMAX] = {0,0,0,0};
   double temp_T[TRADESPERNAMEMAX] = {0,0,0,0};
   ...
   bool const b_aroundNewBar = (int)MathRound(MathMod(Minute() + Hour()*60,timeFrame))>timeFrame-5 || (int)MathRound(MathMod(Minute() + Hour()*60,timeFrame))<5; // 5 min before bar end and 5 min after: TRUE
...
if (Minute()=30 && Seconds()<15) {
	for(int i=0; i<i_namesNumber; i++) {
		if (m_sequence[i][0]>0) {
			Alert("Recap: The slow filter for ",m_names[i],"(",m_myMagicNumber[i],") is ",NormalizeDouble(m_sequence[i][0],(int)MarketInfo(m_names[i],MODE_DIGITS)),
				  ...
				  " Bollinger deviation: ",NormalizeDouble(m_bollingerDeviationInPips[i],0)," and FF: ",NormalizeDouble(m_sequence[i][6],0));
		}
	}
}
...
if ((int)MathFloor(GlobalVariableGet("gv_productMagicNumber")/100)==i_stratMagicNumber) {
	...
	if (GlobalVariableGet("gv_tradesNumber")>1.5) { 
	   ...
	   Alert("The trades number for ",m_names[temp_i]," was changed to ",NormalizeDouble(m_tradesNumber[temp_i],0));
	   GlobalVariableSet("gv_tradesNumber",1);
	}
	...
	// recap alert
	Alert("Recap: The slow filter for ",m_names[temp_i],"(",GlobalVariableGet("gv_productMagicNumber"),") is ",NormalizeDouble(m_sequence[temp_i][0],(int)MarketInfo(m_names[temp_i],MODE_DIGITS)),
	      ...
	      " Bollinger deviation: ",NormalizeDouble(m_bollingerDeviationInPips[temp_i],0)," and FF: ",NormalizeDouble(m_sequence[temp_i][6],0));
	// resetting
   GlobalVariableSet("gv_productMagicNumber",-1);
}
// GET A SNAPSHOT
...
// CLOSING ORDERS //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 // OPENING ORDERS //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      ...
      if (!(b_noNewSequence && m_sequence[i][0]<0)) {  // Send order when no new sequence flag is off 
         for(int k=0; k<m_tradesNumber[i]; k++) {
			...
			if (m_open[i][k]>0 || m_open[i][k]<0) {
				...
				if (m_tradesNumber[i]>1) { m_lots[i][k] = NormalizeDouble(f_lotUnsplit / m_tradesNumber[i],2); }
				else { m_lots[i][k] = f_lotUnsplit; }
			}
  ...
  bool writeOutputToFile(string sequenceFileName, string ticketFileName, string cumLossesFileName)
  {
      ...
      if (h!=INVALID_HANDLE) {
         for(int i=0; i<i_namesNumber; i++) {
            FileWrite(h,m_names[i],",",m_ticket[i][0],",",m_ticket[i][1],",",m_ticket[i][2],",",m_ticket[i][3]);
         } 
         FileClose(h); 
      }
      ...
      if (h!=INVALID_HANDLE) {
         for(int i=0; i<i_namesNumber; i++) {
            FileWrite(h,m_names[i],",",m_cumLosses[i][0],",",m_cumLosses[i][1],",",m_cumLosses[i][2],",",m_cumLosses[i][3]);
         } 
         FileClose(h); 
      }
  ...
  double cumulativePnL()
  {
      double pnl = 0;
      for(int i=0; i<i_namesNumber; i++) { 
         pnl = pnl + m_cumLosses[i][0] + m_cumLosses[i][1] + m_cumLosses[i][2] + m_cumLosses[i][3];
      } 
      return pnl;
  }
