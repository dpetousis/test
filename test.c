...
enum INPUTFILENAME {
	TF_DEMO_H1_TRENDSTALKER = 0, //TF_DEMO_H1_TRENDSTALKER
	TF_DEMO_H4_TRENDSTALKER = 1, //TF_DEMO_H4_TRENDSTALKER
	TF_REAL_H1_TRENDSTALKER = 2, //TF_REAL_H1_TRENDSTALKER
	TF_REAL_H4_TRENDSTALKER = 3, //TF_REAL_H4_TRENDSTALKER
}
enum ACCOUNTCOMPANY {
	THINKMARKETS = 0;
	UOB = 1;
...
input INPUTFILENAME inputFilenameIndex = TF_DEMO_H1_TRENDSTALKER; 
input double const bollinger_deviations = 2;
input BOLLINGERMODE bollinger_mode = UPPER;
input int const bollinger_delay = 5;
int const i_stratMagicNumber = (10*Period()/60) + bollinger_mode;
...
// trading parameters
//string const m_inputFileName[4] = {"TF_DEMO_H1_TRENDSTALKER.txt","TF_DEMO_H4_TRENDSTALKER.txt","TF_REAL_H1_TRENDSTALKER.txt","TF_REAL_H4_TRENDSTALKER.txt"};
double const m_percWarp[1] = { 0.3 };
double const m_percSL[1] = { 0.5 };
...
// credit parameters
double const f_creditPenalty = 25.0;
...
int OnInit()
  {
   ...
   int temp_tickets[2];
   int i_openSequenceForProduct=-1;
   string s_namesRemoved = "", s_namesOpen = "",string s_symbolAppendix = "";
   ...
   switch (AccountCompany()) {
	  case THINKMARKETS: if (IsDemo) { s_symbolAppendix = "pro"; } else { s_symbolAppendix = "algo"; } break;
	  case UOB: s_symbolAppendix = "#"; break;
	  default: s_symbolAppendix = "";
   }
   for(int i=0; i<i_namesNumber; i++) {
   ...
   Alert("This run is for strategy #",i_stratMagicNumber," using mode:",bollinger_mode," and file: ",EnumToString(inputFilenameIndex)+".txt");
...
if ((int)MathFloor(GlobalVariableGet("gv_productMagicNumber")/100)==i_stratMagicNumber) {
	...
	if (GlobalVariableGet("gv_resetSequence")>0.5) { 
	   m_sequence[temp_i][0] = -1;
		...
		m_sequence[temp_i][3] = m_percWarp[0];
		m_sequence[temp_i][4] = m_percSL[0];
		m_sequence[temp_i][5] = m_percTP[0];
...
if ((int)MathFloor(GlobalVariableGet("gv_newSequenceForProduct")/100)==i_stratMagicNumber) {
   ...
   // resetting
   GlobalVariableSet("gv_newSequenceForProduct",-1);
   GlobalVariableGet("gv_creditAmount",0.0);
   GlobalVariableGet("gv_slowFilter",0); 
}
      ...
      if (m_tradeFlag[i]==true) {
		...
		if (m_sequence[i][0]<0 || (m_sequence[i][2]<1.5 && iBars(m_names[i],timeFrame)==m_lastTicketOpenTime[i][0])) {	// no sequence or new sequence opened this bar                      // SF
			...
			f_bollingerBand = MathMax(iBands(m_names[i],timeFrame,(int)m_filter[i][0],bollinger_deviations,0,0,bollinger_mode,1+bollinger_delay),
								iBands(m_names[i],timeFrame,(int)m_filter[i][0],bollinger_deviations,0,0,MODE_CENTRAL,1+bollinger_delay) + 0.25*m_bandsTSAvg[i]*MarketInfo(m_names[i],MODE_POINT));
			f_bollingerBandPrev = MathMin(iBands(m_names[i],timeFrame,(int)m_filter[i][0],bollinger_deviations,0,0,bollinger_mode,2+bollinger_delay),
								iBands(m_names[i],timeFrame,(int)m_filter[i][0],bollinger_deviations,0,0,MODE_CENTRAL,2+bollinger_delay) - 0.25*m_bandsTSAvg[i]*MarketInfo(m_names[i],MODE_POINT)); 
          ...
          if (temp_T[0] < 0 || temp_T[1] < 0) {
			...
			if (f_fastFilter > m_slowFilter[i]) { m_signal[i] = 2; }
			else if (f_fastFilter < m_slowFilter[i]) { m_signal[i] = -2; }
      ...
      if (!(b_noNewSequence && m_sequence[i][0]<0)) {  // Send order when no new sequence flag is off 
         for(int k=0; k<m_tradesNumber[i]; k++) {
			...
			if (m_open[i][k]>0 || m_open[i][k]<0) {
				...
				else { m_lots[i][k] = NormalizeDouble(f_lotUnsplit,m_lotDigits[i]); }
			}
            ...
            if (m_open[i][k]>0) {                                       // criterion for opening Buy
               ...
               s_comment = StringConcatenate(IntegerToString(m_myMagicNumber[i]),s_tradeFlag,"_",DoubleToStr(m_slowFilter[i],(int)MarketInfo(m_names[i],MODE_DIGITS)),
												s_adjFlag,"_",DoubleToStr(-m_loss[i],2),"_",DoubleToStr(m_sequence[i][2]+(k==0)?1:0,0));
         ...
         if (m_open[i][k]<0 || m_open[i][k]>0) {
            ...
            if (ticket < 0)     {                 
			  ...
			  if (i==i_openSequenceForProduct) { i_openSequenceForProduct = -1; }		// if there is an manual open request, reset the flag after trade opens.
            }
         }
  ...
  double accCcyFactor(string symbol, string appendix)
  {
      ...
      **/
      ...
      if (StringCompare(StringSubstr(symbol,0,3),AccountCurrency(),false)==0) {     // ACCXXX
         ...
         else {      // non-currency products VS USD
