...
input string const s_symbolAppendix = "";	// algo, pro or blank
...
int OnInit()
  {
   ...
   Alert("This run is for strategy #",i_stratMagicNumber," using mode:",bollinger_mode," and file: ",s_inputFileName);
...
if ((int)MathFloor(GlobalVariableGet("gv_newSequenceForProduct")/100)==i_stratMagicNumber) {
   ...
   if (m_sequence[temp_i][0]>0) { Alert("A sequence for ",m_names[temp_i],"(",(int)NormalizeDouble(GlobalVariableGet("gv_productMagicNumber"),0),") is already open."); }
      ...
      if (GlobalVariableGet("gv_slowFilter")>0 && GlobalVariableGet("gv_slowFilterGap")>=0 && GlobalVariableGet("gv_bollingerDevInPips")>0) {
         m_sequence[temp_i][0] = GlobalVariableGet("gv_slowFilter");
   ...
   GlobalVariableGet("gv_slowFilterGap",-1); // Actual value not pips, this is defined as the distance from the central SF to lower or upper.
   GlobalVariableGet("gv_bollingerDevInPips",0);
   GlobalVariableGet("gv_tradesNumber",1);
}
...
// CLOSING ORDERS //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 // OPENING ORDERS //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  bool writeOutputToFile(string sequenceFileName, string ticketFileName, string cumLossesFileName)
  {
      ...
      if (h!=INVALID_HANDLE) {
         for(int i=0; i<i_namesNumber; i++) { 
            FileWrite(h,m_names[i],",",m_tradesNumber[i],",",m_sequence[i][0],",",NormalizeDouble(m_sequence[i][1],2),",",m_sequence[i][2],",",m_sequence[i][3],",",m_sequence[i][4],",",m_sequence[i][5],",",m_sequence[i][6],",",m_bollingerDeviationInPips[i],",",m_slowFilterGap[i]); 
         } 
         FileClose(h); 
      }
  ...
  double accCcyFactor(string symbol)
  {
      ...
      **/
      ...
      if (StringCompare(StringSubstr(symbol,0,3),AccountCurrency(),false)==0) {     // ACCXXX
         ...
         string k = StringConcatenate(getName(StringSubstr(symbol,3,3),AccountCurrency()),s_symbolAppendix);
         if (StringCompare(k,"NONE",false)!=0) {
            ...
            k = StringConcatenate(getName("USD",AccountCurrency()),s_symbolAppendix);
