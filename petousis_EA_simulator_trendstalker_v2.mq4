...
int OnInit()
  {
   ...
   ArrayInitialize(m_mirrorTradeFlag,true);
   ArrayInitialize(m_percMirrorCover,0.8);
   ...
   for(int i=0; i<i_namesNumber; i++) {
		...
		for (int j=0;j<2;j++) { 
			if (temp_ticket[j]>0) {
				if (readTradeComment(temp_ticket[j],m_names[i],temp_sequence)) {
					if (temp_sequence[6]<-0.00001) {					// mirror test negative
					...
					else if (temp_sequence[6]>0.00001) {		// mirror test positive
      ...
      if ((f_barSizeTSAvg/m_bandsTSAvg[i] > 1 /**0.11**/) && (m_sequence[i][0]<0)) {        // if noisy and no sequence already live -> exclude pair
         m_tradeFlag[i] = false;
...
// VARIABLE DECLARATIONS /////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   bool m_isPositionPending[],m_isMirrorPositionPending[];
   int m_positionDirection[];
   int m_lastTicketOpenTime[];
   double m_fastFilter[];
   double temp_sequence[7],mirror_sequence[7];
...
// PRELIMINARY PROCESSING ///////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   ArrayResize(m_isMirrorPositionPending,i_namesNumber,0);
   ...
   ArrayInitialize(m_isMirrorPositionPending,false);
      	...
      	if (m_ticket[i][0]>0) {
				...
				if ((OrderCloseTime()>0 && m_ticket[i][1]==0) || (OrderCloseTime()>0 && mirror_orderCloseTime>0)) {			// if closed  - or both closed if mirror exists
					...
					if (readTradeComment(m_ticket[i][0],m_names[i],temp_sequence)) {
						...
						m_isMirrorPositionPending[i] = false;
					...
					if (mirror_orderType>1) { m_isMirrorPositionPending[i] = true; b_pending = true; } 
				}
			}
...
// CLOSING ORDERS //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 if (m_tradeFlag[i]==true) {
      if (m_signal[i]<0 && m_positionDirection[i]==1) {
			...
			if (m_ticket[i][1]>0) {
				res = OrderSelect(m_ticket[i][1],SELECT_BY_TICKET);
				if (m_isMirrorPositionPending[i]==true) {
 ...
 // OPENING ORDERS //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   for(int i=0; i<i_namesNumber; i++) {
   if (m_tradeFlag[i]==true) {
      if ((m_isPositionPending[i]==true || m_isMirrorPositionPending[i]==true) && m_signal[i]==0) {     // if pending order exists -> modify pending order
       		RefreshRates(); 
      ...
      if (((m_signal[i]>0) || (m_signal[i]<0)) && !(b_noNewSequence && m_sequence[i][0]<0))   // Send order when receive buy or sell signal 
        {
         ...
         if (m_signal[i]>0) 
           {                                       // criterion for opening Buy
            ...
            SL=NormalizeDouble(ASK - m_bollingerDeviationInPips[i]*MarketInfo(m_names[i],MODE_POINT),(int)MarketInfo(m_names[i],MODE_DIGITS));     // Calculating SL of opened
            TP=NormalizeDouble(ASK + m_bollingerDeviationInPips[i]*MarketInfo(m_names[i],MODE_POINT),(int)MarketInfo(m_names[i],MODE_DIGITS));   // Calculating TP of opened
			if (m_mirrorTradeFlag[i] && m_isMirrorPositionPending[i]==false) {
				mirror_BID = MarketInfo(m_names[i],MODE_BID);
				mirror_SL=NormalizeDouble(mirror_BID + m_bollingerDeviationInPips[i]*MarketInfo(m_names[i],MODE_POINT),(int)MarketInfo(m_names[i],MODE_DIGITS));     // Calculating SL of opened
				mirror_TP=NormalizeDouble(mirror_BID - m_bollingerDeviationInPips[i]*MarketInfo(m_names[i],MODE_POINT),(int)MarketInfo(m_names[i],MODE_DIGITS));   // Calculating TP of opened
            }
            if (m_isPositionPending[i]==false && m_isPositionOpen[i]==false) {       // if no position and no pending -> send pending order
               // LOTS
				...
				if (m_mirrorTradeFlag[i] && m_isMirrorPositionPending[i]==false) { mirror_lots = NormalizeDouble(m_percMirrorCover[i] * m_lots[i],m_lotDigits[i]); }
         	    // COMMENT
               if (m_sequence[i][0] < 0) { 
               ...
               if (m_mirrorTradeFlag[i] && m_isMirrorPositionPending[i]==false) { mirror_comment = StringConcatenate(IntegerToString(m_myMagicNumber[i]),"M_",DoubleToStr(temp_vwap,(int)MarketInfo(m_names[i],MODE_DIGITS)),s_adjFlag,"_",DoubleToStr(-f_loss,2),"_",DoubleToStr(m_sequence[i][2]+1,0)); }
			   // ORDERSEND
