	  ...
	  if (m_sequenceEndedFlag[i]) {
		...
		if (m_state[i,0]<2 && m_state[i,1]<2 && m_state[i,2]<2 && m_state[i,3]<2) {		
		 ...
		 // there should always be 4 orders (pending or not) unless we hit TP in which case m_sequenceEndedFlag[i]=true
			...
			if (m_state[i,0]==0 && m_state[i,1]>0 ) {m_signal[i,0] = 1; }		// open pending
 ...
 // OPENING ORDERS //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   for (int j=0; j<4; j++) {
   	...
   	if (m_open[i,j]==true) {
		...
		// send ticket
		i_ticket=OrderSend(m_names[i],temp_orderType,temp_lots,m_openPrice[i,j],slippage,m_stopLoss[i,j],m_takeProfit[i,j],s_comment,m_magicNumber[i,j]); 
