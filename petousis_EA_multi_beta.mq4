...
#define TRADESPERNAMEMAX 6

// INPUTS ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//input switches & constants
extern bool b_noNewSequence = false;
input string const s_inputFileName = "TF_DEMO_H1_TRENDSTALKER.txt"; 
input bool const b_trendstalker = true; // 1:trendstalker 0:volstalker
double const m_percWarp[2] = { 0.3, 0.35 };
double const m_percSL[2] = { 1, 0.32 };
double const m_percTP[2] = { 1, 1 };
double const f_percAvBandSeparation = 1;
// for volstalker only
double f_percPipWarp = 1;  // later reset to 1 if running trendstalker
double f_percPnlWarp = 1.333;
/////////////////////// 
double const f_lotPerTradeMax = 0.03;
...
// always positive
input int const i_stratMagicNumber = 10; 
int const i_bandsHistory = 2000;
// filter 
int const filter_history = 50;
input double const bollinger_deviations = 2;
...
double const f_barSizePortion = 0.25;
string const s_symbolAppendix = "";

// TRADE ACCOUNTING VARIABLES ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
int i_namesNumber=0;
bool b_accountingWriteDone=false;
int m_myMagicNumber[NAMESNUMBERMAX];  // Magic Numbers
double m_lots[][TRADESPERNAMEMAX];
double m_accountCcyFactors[NAMESNUMBERMAX];
// 0:SF,1:Losses excl current,2:#,3:Mirror losses,4:mirror #,5:warp,6:mirror warp,7:SL,8:mirror SL,9:TP,10:mirror TP,11:FF freq
double m_sequence[NAMESNUMBERMAX][12];  
////////////////////////////////////////////////////////////////////////////////////////////////   
string m_names[NAMESNUMBERMAX];
string const m_commentTradeFlag[TRADESPERNAMEMAX] = {"A","B","C","D","E","F"};
...
double m_cumLosses[][2];
double m_bandsTSAvg[];
double m_creditAmount[];
double f_creditBalance = 0.0;
double m_barTSAvg[];
double m_slowFilterGap[];
int f_ordersTotal = 0;
bool b_pendingGlobal=false;
...
int OnInit()
  {
   ...
   else { b_readLastWeekDataFromTradePopulation = true; }   // this mode means that things like updated slow filters or mirror/split flags will be lost when EA restarts
   ...
   for(int i=0; i<i_namesNumber; i++) {
      ...
      if (temp == ArraySize(m_rows)) {
         ...
         m_profitInAccCcy[i] = f_percPnlWarp * StringToDouble(m_rows[2]);
      ...
      if (b_readLastWeekDataFromTradePopulation) {
         m_sequence[i][0] = -1.0;      //Initialize to -1,0,0,0,0 
         m_sequence[i][5] = m_percWarp[0];
         m_sequence[i][6] = m_percWarp[1];
         m_sequence[i][7] = m_percSL[0];
         m_sequence[i][8] = m_percSL[1];
         m_sequence[i][9] = m_percTP[0];
         m_sequence[i][10] = m_percTP[1];
         m_sequence[i][11] = m_filter[i][1];
         ...
         for(int k=0; k<12; k++) { m_sequence[i][k] = StringToDouble(m_rows[k+2]); }
         m_bollingerDeviationInPips[i] = StringToDouble(m_rows[14]);
      ...
      if (FileIsExist(s_cumLossesFileName,0)) {
         ...
         m_cumLosses[i][0] = StringToDouble(m_rows[1]);
         m_cumLosses[i][1] = StringToDouble(m_rows[2]);
      }
	  ...
	  if (b_statistics && m_tradeFlag[i]) {
		  ...
		  Alert(m_names[i]," Average band in pips: ",m_bandsTSAvg[i]);
		  m_barTSAvg[i] = NormalizeDouble(f_barSizeTSAvg / i_bandsHistory, (int)MarketInfo(m_names[i],MODE_DIGITS));
		  // quarter the bar size: lower SF=SF-m_slowFilterGap, upper SF=SF+m_slowFilterGap
		  m_slowFilterGap[i] = NormalizeDouble(m_barTSAvg[i] * f_barSizePortion, (int)MarketInfo(m_names[i],MODE_DIGITS));
   ...
   for(int i=0; i<i_namesNumber; i++) {
      ...
      Alert("acc factor for: ",m_names[i]," is ",m_accountCcyFactors[i]);
   }
   ...
   GlobalVariableSet("gv_percWarp",-1);
   GlobalVariableSet("gv_percSL",-1);
   GlobalVariableSet("gv_percTP",-1);
   ...
   if (IsDemo() == false) { Alert("THIS IS A REAL RUN"); }
   else { Alert("THIS IS DEMO RUN"); }
...
// VARIABLE DECLARATIONS /////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   uint tick=0;
   bool b_transition=false,b_switchFFException=false,b_sequenceIsStillUnwarped=false,b_tradeClosedBySL=false,res,b_pending=false,b_appliedCredit=false,b_appliedPenalty=false, b_multiPositionOpen=false,b_sequenceCriterion=false;
   double SL,TP,BID,ASK,f_fastFilterPrev=0.0,f_fastFilter=0.0,f_fastFilterPrevCurrent=0.0,f_flat=0,f_central=0.0,f_lotUnsplit=0,f_loss=0,f_pnlSequence=0,f_pnlSingle=0,
   f_bollingerBandPrev = 0.0,f_bollingerBand = 0.0,temp_fastFF=0.0,temp_slowFF=0.0;
   ...
   double temp_sequence[8]={0,0,0,0,0,0,0,0};
   double temp_pnl[2] = {0,0};
   double temp_T[2] = {0,0};
   string s_comment,s_orderSymbol,s_tradeFlag,s_adjFlag="";
   string s_FFFast = "petousis_supersmoother", s_FFSlow = "petousis_decycler", s_FF="";
   ...
   // 5 min before bar end and 5 min after: TRUE
   bool const b_aroundNewBar = (int)MathRound(MathMod(Minute() + Hour()*60,timeFrame))>timeFrame-5 || (int)MathRound(MathMod(Minute() + Hour()*60,timeFrame))<5;
   bool const b_orderTotalChanged = !(f_ordersTotal == OrdersTotal());
   f_ordersTotal = OrdersTotal();
   bool const b_checkEveryXMinute = (Minute() == 10);
...
if (b_aroundNewBar || b_pendingGlobal || b_orderTotalChanged || b_checkEveryXMinute) {
for(int i=0; i<i_namesNumber; i++) {
   if (m_tradeFlag[i]==true) {
      for(int k=0; k<TRADESPERNAMEMAX; k++) {   // we make sure that if TN is decremented before main is closed, other trades will still be accounted for
      	if (m_ticket[i][k]>0) {
	   ...
	   if (!b_multiPositionOpen && i_ticketSum>0.5) {    // order is closed if no open positions but ticket numbers not yet reset
		   if (b_trendstalker) { b_sequenceCriterion = ((m_sequence[i][1]+temp_pnl[0])>=0.01); }
		   else { b_sequenceCriterion = b_tradeClosedBySL; } //(f_pnlSingle<=-0.1*f_percSL*m_profitInAccCcy[i]); } // ie allow loss up to 10% of SL for volstalker before terminating sequence
		   if (b_sequenceCriterion) {	//ie trade sequence closed positive/negative by one cent or penny
				m_sequence[i][0] = -1;
				for(int k=1; k<5; k++) { m_sequence[i][k] = 0; }
				m_sequence[i][5] = m_percWarp[0];
				m_sequence[i][6] = m_percWarp[1];
				m_sequence[i][7] = m_percSL[0];
				m_sequence[i][8] = m_percSL[1];
				m_sequence[i][9] = m_percTP[0];
				m_sequence[i][10] = m_percTP[1];
				m_sequence[i][11] = m_filter[i][1];;    // FF freq
			   ...
			   m_sequence[i][1] = m_sequence[i][1] + temp_pnl[0];
			   m_sequence[i][3] = m_sequence[i][3] + temp_pnl[1];
			}
			for(int k=0; k<TRADESPERNAMEMAX; k++) { // if multi position closed reset all tickets, reset all columns and update cumulative pnl
			   m_ticket[i][k] = 0; 
			   m_cumLosses[i][k] = m_cumLosses[i][k] + temp_pnl[k];
			} 
		}
...
if ((int)MathFloor(GlobalVariableGet("gv_productMagicNumber")/100)==i_stratMagicNumber) {
	...
	if (GlobalVariableGet("gv_fastFilter")>0) { 
	   Alert("The fast filter for product ",m_names[temp_i]," was changed to ",GlobalVariableGet("gv_fastFilter")," from ",m_sequence[temp_i][11]);
	   m_sequence[temp_i][11] = GlobalVariableGet("gv_fastFilter");
	   GlobalVariableSet("gv_fastFilter",-1); 
	}
	/**
	// SETTING EXTERNALLY THE TRADES NUMBER USING GLOBAL VARIABLES /////////////////////////////////////
	if (GlobalVariableGet("gv_tradesNumber")>-0.5) { 
	   m_tradesNumber[temp_i] = (int)NormalizeDouble(GlobalVariableGet("gv_tradesNumber"),0); // you can only increase TN otherwise it will lose trade accounting
	...
	**/
	...
	if (GlobalVariableGet("gv_slowFilter")>0.5 || GlobalVariableGet("gv_slowFilter")<-0.5) { 
	   m_sequence[temp_i][0] = GlobalVariableGet("gv_slowFilter");
		Alert("The slow filter for ",m_names[temp_i]," was changed to ",m_sequence[temp_i][0]," with lower: ",m_sequence[temp_i][0]-m_slowFilterGap[temp_i]," and upper: ",m_sequence[temp_i][0]+m_slowFilterGap[temp_i]," and gap: ",m_slowFilterGap[temp_i]); 
		GlobalVariableSet("gv_slowFilter",0);
	}
	// SETTING EXTERNALLY THE SLOW FILTER GAP VALUE USING GLOBAL VARIABLES /////////////////////////////////////
	// IMPORTANT: THE FILTER GAP WILL REVERT TO THE HISTORICAL VALUE WHEN PROGRAM RESTARTS //////////////////////
	...
	if (MathAbs(GlobalVariableGet("gv_bollingerDevInPips"))>0.99) { 
	   ...
	   Alert("The bollinger deviation for ",m_names[temp_i]," was changed to ",m_bollingerDeviationInPips[temp_i]," with the original being: ",NormalizeDouble(f_percPipWarp*f_percAvBandSeparation*m_bandsTSAvg[temp_i],0)); 
	   GlobalVariableSet("gv_bollingerDevInPips",0);
	}
	// SETTING EXTERNALLY THE WARP,SL AND TP PERCENTAGES /////////////////////////////////////////////////
	// mirror Key 0:main 1:mirror 
	if (GlobalVariableGet("gv_percWarp")>0 && GlobalVariableGet("gv_percSL")>0 && GlobalVariableGet("gv_percTP")>0) {
	   m_sequence[temp_i][5] = GlobalVariableGet("gv_percWarp");
	   m_sequence[temp_i][7] = GlobalVariableGet("gv_percSL");
	   m_sequence[temp_i][9] = GlobalVariableGet("gv_percTP");
		Alert("The % warp,SL and TP for product ",m_names[temp_i]," was changed to ",m_sequence[temp_i][5],",",m_sequence[temp_i][7]," and ",m_sequence[temp_i][9]); 
		GlobalVariableSet("gv_percWarp",-1);
		GlobalVariableSet("gv_percSL",-1);
		GlobalVariableSet("gv_percTP",-1);
	}
	// recap alert
	Alert("Recap: The slow filter for ",m_names[temp_i],",",GlobalVariableGet("gv_productMagicNumber")," is ",m_sequence[temp_i][0]," with lower: ",m_sequence[temp_i][0]-m_slowFilterGap[temp_i]," , upper: ",m_sequence[temp_i][0]+m_slowFilterGap[temp_i],", gap: ",m_slowFilterGap[temp_i]," Bollinger deviation: ",m_bollingerDeviationInPips[temp_i]," and FF: ",m_sequence[temp_i][11]);
	// resetting
   GlobalVariableSet("gv_productMagicNumber",-1);
}
// GET A SNAPSHOT
...
// Make sure rest of ontimer() does not run continuously when not needed
   if (b_aroundNewBar || b_pendingGlobal) {
      ...
      if (m_tradeFlag[i]==true) {
	      ...
	      if (m_sequence[i][0]<0 || (m_sequence[i][2]<1.5 && iBars(m_names[i],timeFrame)==m_lastTicketOpenTime[i][0])) {	// no sequence or new sequence opened this bar                                                       // SF
	         ...
	         f_fastFilterPrev = iCustom(m_names[i],0,"petousis_decycler",m_filter[i][1],filter_history,1,2);
	 	      f_bollingerBand = iBands(m_names[i],timeFrame,(int)m_filter[i][0],bollinger_deviations,0,0,bollinger_mode,1+bollinger_delay);
	 	      f_bollingerBandPrev = iBands(m_names[i],timeFrame,(int)m_filter[i][0],bollinger_deviations,0,0,bollinger_mode,2+bollinger_delay); 
	         ...
	         else if (m_sequence[i][2]>=2  && (int)m_sequence[i][11]==(int)m_filter[i][1]) {
          ...
          if (temp_T[0] < 0 || temp_T[1] < 0) {
            //b_sequenceIsStillUnwarped = (MathAbs(m_sequence[i][1])<m_profitInAccCcy[i]*f_percWarp);
            if (m_sequence[i][0]<0) {         // either new sequence or manually set (-1) mid-sequence, so update the pips
               //f_central = iBands(m_names[i],timeFrame,(int)m_filter[i][0],bollinger_deviations,0,0,MODE_MAIN,1);
	            ...
	            if (f_fastFilter>f_bollingerBand) { // && m_positionDirection[i][0]<1 && iBars(m_names[i],timeFrame)>m_lastTicketOpenTime[i][0]) {
                  // when on free moving slow filter only enter buy on upper and sell on lower.
                  if (bollinger_mode==1) {
                     if (b_trendstalker) { m_signal[i] = 2; } else { m_signal[i] = -2; }
                  }
                  else { m_signal[i] = 0; }
               ...
               else if (f_fastFilter<f_bollingerBand) { // && m_positionDirection[i][0]>-1 && iBars(m_names[i],timeFrame)>m_lastTicketOpenTime[i][0]) {
                  // when on free moving slow filter only enter buy on upper and sell on lower.
                  if (bollinger_mode==2) {
                     if (b_trendstalker) { m_signal[i] = -2; } else { m_signal[i] = 2; }
                  }
               ...
               if (f_fastFilter>f_bollingerBand+m_slowFilterGap[i]) { // && m_positionDirection[i][0]<1 && iBars(m_names[i],timeFrame)>m_lastTicketOpenTime[i][0]) {
                  if (b_trendstalker) { m_signal[i] = 2; } else { m_signal[i] = -2; }
                  // Use fast filter to set slow filter level if 1) new sequence or 2) change larger than f_adjustLevel
   	         ...
   	         else if (f_fastFilter<f_bollingerBand-m_slowFilterGap[i]) { // && m_positionDirection[i][0]>-1 && iBars(m_names[i],timeFrame)>m_lastTicketOpenTime[i][0]) {
                  if (b_trendstalker) { m_signal[i] = -2; } else { m_signal[i] = 2; }
                  // Use fast filter to set slow filter level if 1) new sequence or 2) change larger than f_adjustLevel
                  if (m_sequence[i][0]>0 && (MathAbs(m_sequence[i][0]-f_fastFilter)>f_adjustLevel*m_bollingerDeviationInPips[i]*MarketInfo(m_names[i],MODE_POINT))) { 
            			m_slowFilter[i] = f_fastFilter + MarketInfo(m_names[i],MODE_POINT); 
...
// ESTABLISH LOTS AND # OF TRADES TO OPEN /////////////////////////////////////////////////////////////////////////////////////////////////////////
for(int i=0; i<i_namesNumber; i++) {
   if (m_tradeFlag[i]==true && (m_signal[i]>1 || m_signal[i]<-1)) {
      // Loss per name
      if ((-m_sequence[i][1]<f_creditPenaltyThreshold && f_creditBalance>0)) { // apply penalty with credit balance
         ...
         m_loss[i] = -m_sequence[i][1]; 
   	   m_appliedCreditFlag[i] = false;
   	   m_appliedPenaltyFlag[i] = false;
      }
      // Lots per trade and # of trades
      f_lotUnsplit = NormalizeDouble(MathMax(m_lotMin[i], MathMax(m_profitInAccCcy[i],MathAbs(m_loss[i])/m_sequence[i][5]) / m_accountCcyFactors[i] / m_bollingerDeviationInPips[i]),m_lotDigits[i]);
      ...
      for(int k=0; k<TRADESPERNAMEMAX; k++) {
         m_lots[i][k] = MathMin(f_lotPerTradeMax,MathMax(0,f_lotUnsplit-k*f_lotPerTradeMax));
      }
   }
}      
...
for(int i=0; i<i_namesNumber; i++) {
   if (m_tradeFlag[i]==true && m_signal[i]!=0) {
      for(int k=0; k<TRADESPERNAMEMAX; k++) {
         ...
         else if (m_signal[i]>1 && m_lots[i][k]>0.009 && m_positionDirection[i][k]==0 && iBars(m_names[i],timeFrame)>m_lastTicketOpenTime[i][0]) { m_open[i][k]=1; }
...
// CLOSING ORDERS //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 // OPENING ORDERS //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
         ...
         if (m_isPositionPending[i][k]==true && m_open[i][k]==0) {     // if pending order exists -> modify pending order
         		...
         		if (m_positionDirection[i][k]==1) { 
         			ASK = MarketInfo(m_names[i],MODE_ASK)-MarketInfo(m_names[i],MODE_STOPLEVEL)*MarketInfo(m_names[i],MODE_POINT);
					   SL=NormalizeDouble(ASK - m_sequence[i][7+k] * m_bollingerDeviationInPips[i]*MarketInfo(m_names[i],MODE_POINT),(int)MarketInfo(m_names[i],MODE_DIGITS));
                  TP=NormalizeDouble(ASK + m_sequence[i][9+k] * m_bollingerDeviationInPips[i]*MarketInfo(m_names[i],MODE_POINT),(int)MarketInfo(m_names[i],MODE_DIGITS));   // Calculating TP of opened
            		...
            		SL=NormalizeDouble(BID + m_sequence[i][7+k]*m_bollingerDeviationInPips[i]*MarketInfo(m_names[i],MODE_POINT),(int)MarketInfo(m_names[i],MODE_DIGITS));
					   TP=NormalizeDouble(BID - m_sequence[i][9+k]*m_bollingerDeviationInPips[i]*MarketInfo(m_names[i],MODE_POINT),(int)MarketInfo(m_names[i],MODE_DIGITS));   // Calculating TP of opened
      ...
      if (!(b_noNewSequence && m_sequence[i][0]<0)) {  // Send order when no new sequence flag is off 
         for(int k=0; k<TRADESPERNAMEMAX; k++) {
            ...
            if (m_open[i][k]>0) {                                       // criterion for opening Buy
               ...
               TP=NormalizeDouble(ASK + m_sequence[i][9+k]*m_bollingerDeviationInPips[i]*MarketInfo(m_names[i],MODE_POINT),(int)MarketInfo(m_names[i],MODE_DIGITS));   // Calculating TP of opened
         	    ...
         	    // COMMENT
               if (m_sequence[i][k==0 ? 2 : 4]>1 && MathAbs(m_sequence[i][0]-m_slowFilter[i])>MarketInfo(m_names[i],MODE_POINT)) { s_adjFlag = "A"; }	// update if last move too big
      		   else { s_adjFlag = ""; }
               switch (m_tradesNumber[i]) {
                  case 1: s_tradeFlag = ""; break;
                  default: s_tradeFlag = m_commentTradeFlag[k];
               }
               s_comment = StringConcatenate(IntegerToString(m_myMagicNumber[i]),s_tradeFlag,"_",DoubleToStr(m_slowFilter[i],(int)MarketInfo(m_names[i],MODE_DIGITS)),s_adjFlag,"_",DoubleToStr(-m_loss[i],2),"_",DoubleToStr(m_sequence[i][k==0 ? 2 : 4]+1,0));
         ...
         if (m_open[i][k]<0 || m_open[i][k]>0) {
            ...
            if (ticket < 0)     {                 
               ...
               Alert("Loss: ",-f_loss,". Factor: ",m_accountCcyFactors[i],". Pips: ",m_bollingerDeviationInPips[i],". SL: ",SL,". TP: ",TP);
            }
            else {				// Success :)
  ...
  bool writeOutputToFile(string sequenceFileName, string ticketFileName, string cumLossesFileName)
  {
      ...
      if (h!=INVALID_HANDLE) {
         for(int i=0; i<i_namesNumber; i++) { 
            FileWrite(h,m_names[i],",",m_tradesNumber[i],",",m_sequence[i][0],",",m_sequence[i][1],",",m_sequence[i][2],",",m_sequence[i][3],",",m_sequence[i][4],",",m_sequence[i][5],",",m_sequence[i][6],",",m_sequence[i][7],",",m_sequence[i][8],",",m_sequence[i][9],",",m_sequence[i][10],",",m_sequence[i][11],",",m_bollingerDeviationInPips[i]); 
         } 
         FileClose(h); 
      }
      ...
      if (h!=INVALID_HANDLE) {
         for(int i=0; i<i_namesNumber; i++) {
            FileWrite(h,m_names[i],",",m_cumLosses[i][0],",",m_cumLosses[i][1]);
         } 
         FileClose(h); 
      }
  ...
  double cumulativePnL()
  {
      double pnl = 0;
      for(int i=0; i<i_namesNumber; i++) { 
         pnl = pnl + m_cumLosses[i][0] + m_cumLosses[i][1];
      } 
      return pnl;
  }
  ...
  double accCcyFactor(string symbol)
  {
      ...
      **/
      ...
      if (StringCompare(StringSubstr(symbol,0,3),AccountCurrency(),false)==0) {
         ...
         string k = getName(StringSubstr(symbol,3,3),AccountCurrency());
         if (StringCompare(k,"NONE",false)!=0) {
            if (StringFind(k,AccountCurrency())==0) {
	    	      if (StringFind(k,StringConcatenate(AccountCurrency(),"JPY"))>-1) { result = 100 / MarketInfo(StringConcatenate(k,s_symbolAppendix),MODE_BID); }
		         else { result = 1.0 / MarketInfo(StringConcatenate(k,s_symbolAppendix),MODE_BID); }
	         }
            else if (StringFind(k,AccountCurrency())==3) { result = MarketInfo(StringConcatenate(k,s_symbolAppendix),MODE_BID); } 
         } 
         else {
