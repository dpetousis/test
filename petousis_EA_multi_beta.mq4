...
double const m_percWarp[1] = { 0.3 };
double const m_percSL[1] = { 1 };
double const m_percTP[1] = { 1 };
...
// always positive
input int const i_stratMagicNumber = 99; 
...
// 0:SF,1:Losses excl current,2:#,3:warp,4:SL,5:TP,6:FF freq
double m_sequence[NAMESNUMBERMAX][7];  
////////////////////////////////////////////////////////////////////////////////////////////////   
...
double m_cumLosses[][TRADESPERNAMEMAX];
double m_bandsTSAvg[];
...
int OnInit()
  {
   ...
   for(int i=0; i<i_namesNumber; i++) {
      ...
      if (b_readLastWeekDataFromTradePopulation) {
         ...
         m_sequence[i][3] = m_percWarp[0];
         m_sequence[i][4] = m_percSL[0];
         m_sequence[i][5] = m_percTP[0];
      ...
      if (FileIsExist(s_cumLossesFileName,0)) {
         ...
         for(int k=0; k<TRADESPERNAMEMAX; k++) { m_cumLosses[i][k] = StringToDouble(m_rows[k+1]); }
      }
...
// VARIABLE DECLARATIONS /////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   double SL,TP,BID,ASK,f_fastFilterPrev=0.0,f_fastFilter=0.0,f_fastFilterPrevCurrent=0.0,f_flat=0,f_central=0.0,f_lotUnsplit=0,f_pnlSequence=0,f_pnlSingle=0,
   f_bollingerBandPrev = 0.0,f_bollingerBand = 0.0,temp_fastFF=0.0,temp_slowFF=0.0,temp_pnlSum=0.0;
   ...
   double temp_pnl[TRADESPERNAMEMAX] = {0,0,0,0,0,0};
   double temp_T[TRADESPERNAMEMAX] = {0,0,0,0,0,0};
   ...
   if (m_tradeFlag[i]==true) {
      for(int k=0; k<TRADESPERNAMEMAX; k++) {   // we make sure that if TN is decremented before main is closed, other trades will still be accounted for
	   ...
	   if (!b_multiPositionOpen && i_ticketSum>0.5) {    // order is closed if no open positions but ticket numbers not yet reset
		   ...
		   b_sequenceCriterion = ((m_sequence[i][1]+temp_pnlSum)>=0.01);
...
if ((int)MathFloor(GlobalVariableGet("gv_productMagicNumber")/100)==i_stratMagicNumber) {
	...
	if (MathAbs(GlobalVariableGet("gv_bollingerDevInPips"))>0.99) { 
	   ...
	   Alert("The bollinger deviation for ",m_names[temp_i]," was changed to ",m_bollingerDeviationInPips[temp_i]," with the original being: ",NormalizeDouble(f_percAvBandSeparation*m_bandsTSAvg[temp_i],0)); 
	   GlobalVariableSet("gv_bollingerDevInPips",0);
	}
	// recap alert
	Alert("Recap: The slow filter for ",m_names[temp_i],",",GlobalVariableGet("gv_productMagicNumber"),
	      " is ",NormalizeDouble(m_sequence[temp_i][0],(int)MarketInfo(m_names[temp_i],MODE_DIGITS)),
	      ...
	      " Bollinger deviation: ",NormalizeDouble(m_bollingerDeviationInPips[temp_i],0)," and FF: ",NormalizeDouble(m_sequence[temp_i][11],0));
	// resetting
   GlobalVariableSet("gv_productMagicNumber",-1);
}
// GET A SNAPSHOT
            ...
            if (m_sequence[i][0]<0) {         // either new sequence or manually set (-1) mid-sequence, so update the pips
               //f_central = iBands(m_names[i],timeFrame,(int)m_filter[i][0],bollinger_deviations,0,0,MODE_MAIN,1);
	            ...
	            if (f_fastFilter>f_bollingerBand) { // && m_positionDirection[i][0]<1 && iBars(m_names[i],timeFrame)>m_lastTicketOpenTime[i][0]) {
                  ...
                  if (bollinger_mode==1) { m_signal[i] = 2; }
                  else { m_signal[i] = 0; }
                  ...
                  // when on free moving slow filter only enter buy on upper and sell on lower.
                  if (bollinger_mode==2) { m_signal[i] = -2; }
                  else { m_signal[i] = 0; }
                  // Use fast filter to set slow filter level if 1) new sequence or 2) change larger than f_adjustLevel
...
for(int i=0; i<i_namesNumber; i++) {
   if (m_tradeFlag[i]==true && (m_signal[i]>1 || m_signal[i]<-1)) {
      ...
      for(int k=0; k<TRADESPERNAMEMAX; k++) {
         m_lots[i][k] = NormalizeDouble(MathMin(f_lotPerTradeMax,MathMax(0,f_lotUnsplit-k*f_lotPerTradeMax)),m_lotDigits[i]);
         if (m_lots[i][k]>0.009) { m_tradesNumber[i]=k+1; }
         Alert("Lots unsplit are:",f_lotUnsplit," lots split:",k," is ",m_lots[i][k]); //temporary
      }
   }
}      
...
// CLOSING ORDERS //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 // OPENING ORDERS //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      ...
      if (!(b_noNewSequence && m_sequence[i][0]<0)) {  // Send order when no new sequence flag is off 
         for(int k=0; k<TRADESPERNAMEMAX; k++) {
            ...
            if (m_open[i][k]>0) {                                       // criterion for opening Buy
               ...
               s_comment = StringConcatenate(IntegerToString(m_myMagicNumber[i]),s_tradeFlag,"_",DoubleToStr(m_slowFilter[i],(int)MarketInfo(m_names[i],MODE_DIGITS)),s_adjFlag,"_",DoubleToStr(-m_loss[i],2),"_",DoubleToStr(m_sequence[i][2]+1,0));
  ...
  bool writeOutputToFile(string sequenceFileName, string ticketFileName, string cumLossesFileName)
  {
      ...
      if (h!=INVALID_HANDLE) {
         for(int i=0; i<i_namesNumber; i++) { 
            FileWrite(h,m_names[i],",",m_tradesNumber[i],",",m_sequence[i][0],",",m_sequence[i][1],",",m_sequence[i][2],",",m_sequence[i][3],",",m_sequence[i][4],",",m_sequence[i][5],",",m_sequence[i][6],",",m_bollingerDeviationInPips[i]); 
         } 
         FileClose(h); 
      }
      ...
      if (h!=INVALID_HANDLE) {
         for(int i=0; i<i_namesNumber; i++) {
            FileWrite(h,m_names[i],",",m_ticket[i][0],",",m_ticket[i][1],",",m_ticket[i][2],",",m_ticket[i][3],",",m_ticket[i][4],",",m_ticket[i][5]);
         } 
         FileClose(h); 
      }
      ...
      if (h!=INVALID_HANDLE) {
         for(int i=0; i<i_namesNumber; i++) {
            FileWrite(h,m_names[i],",",m_cumLosses[i][0],",",m_cumLosses[i][1],",",m_cumLosses[i][2],",",m_cumLosses[i][3],",",m_cumLosses[i][4],",",m_cumLosses[i][5]);
         } 
         FileClose(h); 
      }
  ...
  double cumulativePnL()
  {
      double pnl = 0;
      for(int i=0; i<i_namesNumber; i++) { 
         pnl = pnl + m_cumLosses[i][0] + m_cumLosses[i][1] + m_cumLosses[i][2] + m_cumLosses[i][3] + m_cumLosses[i][4] + m_cumLosses[i][5];
      } 
      return pnl;
  }
