 ...
 if (m_tradeFlag[i]==true) {
      if (m_signal[i]<0 && m_positionDirection[i]==1) {
			...
			if (m_isPositionPending[i]==true) {
				...
				res = OrderClose(m_ticket[i],OrderLots(),MarketInfo(m_names[i],MODE_BID),100);          // slippage 100, so it always closes
			}
            if (res==true) {
      ...
      if (m_closeSell[i]==true) {
         if ( m_signal[i]>0 && m_positionDirection[i]==-1) {
			...
			if (m_isPositionPending[i]==true) {
				...
				res = OrderClose(m_ticket[i],OrderLots(),MarketInfo(m_names[i],MODE_ASK),100);
			}
            if (res==true) {
      ...
      if (((m_signal[i]>0) || (m_signal[i]<0)) && !(b_noNewSequence && m_sequence[i][0]<0))                             // Send order when receive buy or sell signal
        {
         ...
         if (m_signal[i]>0) 
           {                                       // criterion for opening Buy
            ...
            else if (m_positionPending[i]==true && m_positionDirection[i]==1) {     // if pending order exists -> modify pending order
         ...
         if (m_signal[i]<0) 
           {                                       // criterion for opening Sell
            ...
            else if (m_positionPending[i]==true && m_positionDirection[i]==-1) {
