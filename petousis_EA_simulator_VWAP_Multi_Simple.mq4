...
int OnInit()
  {
   ...
   for(int i=0; i<i_namesNumber; i++) {
      ...
      **/
      ...
      if (isPositionOpen(m_myMagicNumber[i],m_names[i,0])) {                                                       // check if trade open
         if (readLastTradeSubComment(m_myMagicNumber[i],m_names[i,0],false,temp_sequence)) {
            for (int j=0;j<3;j++) {
               m_sequence[i][j] = temp_sequence[j];
            }
         }
         else { PrintFormat("Cannot read open trade comment %s",m_names[i,0]); }
      }
      else {                                                                                                          // if not open check in closed trades
         if (readLastTradeSubComment(m_myMagicNumber[i],m_names[i,0],true,temp_sequence)) {
         ...
         else { PrintFormat("Cannot read closed trade comment %s",m_names[i,0]); }
      }
      ...
      for(int i=0; i<i_namesNumber; i++) {
         if (b_VWAP) {
            if (true) { // use constant vwap for sequence
               temp_T1 = iCustom(m_names[i,0],0,"petousis_VWAPsignal",StrToInteger(m_names[i,6]),StrToInteger(m_names[i,7]),i_mode,filter_supersmoother,false,f_deviationPerc,1000,m_sequence[i][0],3,1);
               m_VWAP[i] = iCustom(m_names[i,0],0,"petousis_VWAPsignal",StrToInteger(m_names[i,6]),StrToInteger(m_names[i,7]),i_mode,filter_supersmoother,false,f_deviationPerc,1000,m_sequence[i][0],2,1);       //needed at every tick
            }
            else {      // use varying vwap
               temp_T1 = iCustom(m_names[i,0],0,"petousis_VWAPsignal",StrToInteger(m_names[i,6]),StrToInteger(m_names[i,7]),i_mode,filter_supersmoother,false,f_deviationPerc,1000,-1,3,1);
               m_VWAP[i] = iCustom(m_names[i,0],0,"petousis_VWAPsignal",StrToInteger(m_names[i,6]),StrToInteger(m_names[i,7]),i_mode,filter_supersmoother,false,f_deviationPerc,1000,-1,2,1);       //needed at every tick
            }
         }
         else {
            temp_T1 = iCustom(m_names[i,0],0,"petousis_SuperSmootherSignal",StrToInteger(m_names[i,6]),StrToInteger(m_names[i,7]),filter_supersmoother,false,f_deviationPerc,1000,3,1);
            m_VWAP[i] = iCustom(m_names[i,0],0,"petousis_SuperSmootherSignal",StrToInteger(m_names[i,6]),StrToInteger(m_names[i,7]),filter_supersmoother,false,f_deviationPerc,1000,4,1);       //needed at every tick
         }
         ...
         if (temp_T1 > 0.001) {              // previous
            if (temp_T1 > m_VWAP[i]) {
               m_signal[i] = 1;
               // correct this line
               upper = iCustom(m_names[i,0],0,"petousis_VWAPsignal",StrToInteger(m_names[i,6]),StrToInteger(m_names[i,7]),i_mode,filter_supersmoother,false,f_deviationPerc,1000,-1,2,1);
               central =  iCustom(m_names[i,0],0,"petousis_VWAPsignal",StrToInteger(m_names[i,6]),StrToInteger(m_names[i,7]),i_mode,filter_supersmoother,false,f_deviationPerc,1000,-1,2,1);
               ...
               m_signal[i] = -1;
               //correct this line
               lower = iCustom(m_names[i,0],0,"petousis_VWAPsignal",StrToInteger(m_names[i,6]),StrToInteger(m_names[i,7]),i_mode,filter_supersmoother,false,f_deviationPerc,1000,-1,2,1);
               central =  iCustom(m_names[i,0],0,"petousis_VWAPsignal",StrToInteger(m_names[i,6]),StrToInteger(m_names[i,7]),i_mode,filter_supersmoother,false,f_deviationPerc,1000,-1,2,1);
...
// CLOSING ORDERS //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 for(int i=0; i<i_namesNumber; i++) {
      if (m_closeBuy[i]==true) {
         m_barLastOpenedTrade[i] = barLastOpenedTrade(m_myMagicNumber[i],m_names[i,0]);
         if (m_isPositionOpen[i] && m_barLastOpenedTrade[i]!=0) {
            RefreshRates();
            Alert("Attempt to close Buy ",m_orderTickets[i]); 
            res = OrderClose(m_orderTickets[i],m_orderLots[i],MarketInfo(m_names[i,0],MODE_BID),100);          // slippage 100, so it always closes
            if (res==true) {
      ...
      if (m_closeSell[i]==true) {
         ...
         if ( m_isPositionOpen[i] && m_barLastOpenedTrade[i]!=0) {
            ...
            res = OrderClose(m_orderTickets[i],m_orderLots[i],MarketInfo(m_names[i,0],MODE_ASK),100);
            if (res==true) {
...
// OPENING TRADING CRITERIA  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   //if ((TimeHour(TimeCurrent()) >= i_hourStart) && (TimeHour(TimeCurrent()) <= i_hourEnd) && (TimeDayOfWeek(TimeCurrent())!=0) && !((TimeDayOfWeek(TimeCurrent())==5) && (TimeHour(TimeCurrent())>=i_hourEndFriday)))        {    
      for(int i=0; i<i_namesNumber; i++) {
         ...
         if ((temp_signal >0) && StringCompare(m_names[i,1],"Y",false)==0 && b_long){ 
            m_openBuy[i] = true; 
            //Print("indicator Buy signal"); 
         }
         else if ((temp_signal <0) && StringCompare(m_names[i,2],"Y",false)==0 && b_short){ 
            m_openSell[i] = true; 
         ...
         if (m_openBuy[i]==true)  {                            
            ...
            if (m_isPositionOpen[i]==true) { m_openBuy[i]=false; }      // dont buy if position already open
               ...
               m_barLastOpenedTradeHistory[i] = barLastOpenedTradeHistory(m_myMagicNumber[i],m_names[i,0]);
 ...
 // OPENING ORDERS //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   for(int i=0; i<i_namesNumber; i++) {
      if (((m_openBuy[i]==true) || (m_openSell[i]==true)) && !(b_noNewSequence && m_sequence[i][0]<0))                             // Send order when receive buy or sell signal
        {
         m_isPositionOpen[i]=isPositionOpen(m_myMagicNumber[i],m_names[i,0]);
         m_ticketPositionPending[i]=ticketPositionPending(m_myMagicNumber[i],m_names[i,0]);
         ...
         if (OrderSelect(m_ticketPositionPending[i],SELECT_BY_TICKET,MODE_TRADES) && (OrderType()==OP_BUYSTOP || OrderType()==OP_SELLSTOP)) {
            ...
            if (res) { Print(m_names[i,0],"_Preemptive Order deleted successfully, because there was a buy/sell signal"); }
            else { Alert(m_names[i,0],"_Preemptive Order deletion failed. There is a buy/sell signal"); }
         }
         // Open Buy
         if (m_openBuy[i]==true) // && (int)MarketInfo(m_names[i,0],MODE_TRADEALLOWED)>0) 
           {                                       // criterion for opening Buy
            ...
            TP=NormalizeDouble(2*BID - SL,MarketInfo(m_names[i,0],MODE_DIGITS));   // Calculating TP of opened
            ...
            if (m_ticketPositionPending[i]<0 && m_isPositionOpen[i]==false) {       // if no position and no pending -> send pending order
               ...
               if (ticket < 0)  {                  // Success :)   
                  ...
                  if (b_sendEmail) { 
                     res = SendMail("VWAP TRADE ALERT","Algo bought "+m_names[i,0]+" "+DoubleToStr(Period(),0)); 
                     if (res==false) { Alert(m_names[i,0]+" "+DoubleToStr(Period(),0)," Email could not be sent.");  }
                  }
               }
            }
         ...
         if (m_openSell[i]==true) // && (int)MarketInfo(m_names[i,0],MODE_TRADEALLOWED)>0) 
           {                                       // criterion for opening Sell
            ...
            TP=NormalizeDouble(2*ASK - SL,MarketInfo(m_names[i,0],MODE_DIGITS));   // Calculating TP of opened
            m_lots[i] = MathMax(0.01,NormalizeDouble((-m_sequence[i][1]+(SLinUSD provided in m_names)) * m_accountCcyFactors[i] / m_bollingerDeviationInPips[i],2));
            ...
            if (m_ticketPositionPending[i]<0 && m_isPositionOpen[i]==false) {
               ...
               if (ticket < 0)     {                 // Success :)
                  ...
                  PlaySound("bikehorn.wav");
                  if (b_sendEmail) { 
                     res = SendMail("VWAP TRADE ALERT","Algo sold "+m_names[i,0]+" "+DoubleToStr(Period(),0)); 
                     if (res==false) { Alert(m_names[i,0]+" "+DoubleToStr(Period(),0)," Email could not be sent.");  }
                  }
               }
            }
