...
bool b_accountingWriteDone=false,b_pendingGlobal=false,b_openManualSequence=false,b_scheduledAlertSent=false;
...
int OnInit()
  {
   ...
   if (timeFrame == 5) { EventSetTimer(2); }
   ...
   ArrayResize(m_lotMax,i_namesNumber,0);
   ...
   for(int i=0; i<i_namesNumber; i++) {
      ...
      m_lotMax[i] = MarketInfo(m_names[i],MODE_MAXLOT);
   ...
   GlobalVariableSet("gv_takeProfitPerc",-1);
   GlobalVariableSet("gv_newSequenceNotAllowed",-1);
...
// VARIABLE DECLARATIONS /////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   double SL,TP,BID,ASK,f_fastFilterPrev=0.0,f_fastFilter=0.0,f_lotUnsplit=0,f_bollingerBandPrev = 0.0,f_bollingerBand = 0.0,temp_fastFF=0.0,temp_slowFF=0.0,temp_pnlSum=0.0,f_timeInHours=0;
   ...
   // RUN FLAGS
   if (timeFrame == 5) { b_aroundNewBar = (int)MathRound(MathMod(Minute(),timeFrame))>timeFrame-2 || (int)MathRound(MathMod(Minute(),timeFrame))<2; } //  runs 5th(4),1st(0) and 2nd(1) minute of every bar 
   else if (timeFrame == 60) { b_aroundNewBar = (int)MathRound(MathMod(Minute() + Hour()*60,timeFrame))>timeFrame-5 || (int)MathRound(MathMod(Minute() + Hour()*60,timeFrame))<5; } // 5 min before bar end and 5 min after
   else if (timeFrame == 240) { b_aroundNewBar = (int)MathRound(MathMod(Minute() + Hour()*60,60))>60-5 || (int)MathRound(MathMod(Minute() + Hour()*60,60))<5; } 	// still rerun around every hour
...
// PRELIMINARY PROCESSING ///////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   ArrayResize(m_productOutOfHours,i_namesNumber,0);
   ...
   ArrayInitialize(m_productOutOfHours,false);
...
if (Minute()==30 && b_scheduledAlertSent==false) {
	for(int i=0; i<i_namesNumber; i++) {
		if (m_sequence[i][0]>0) {
	...
	b_scheduledAlertSent = true;
}
else if (Minute()==29) { b_scheduledAlertSent = false; }
...
if ((int)MathFloor(GlobalVariableGet("gv_productMagicNumber")/100)==i_stratMagicNumber) {
	...
	// SETTING EXTERNALLY THE TAKE PROFIT % VALUE USING GLOBAL VARIABLES /////////////////////////////////////
	if (GlobalVariableGet("gv_takeProfitPerc")>0) { 
	   m_sequence[temp_i][5] = GlobalVariableGet("gv_takeProfitPerc");
		GlobalVariableSet("gv_takeProfitPerc",-1);
	}
	// STOP NEW SEQUENCES FOR A PRODUCT USING GLOBAL VARIABLES /////////////////////////////////////
	if (GlobalVariableGet("gv_newSequenceNotAllowed")>-0.5) {      // -1: no state, 0: false, 1:true
	   m_newSequenceNotAllowed[temp_i] = GlobalVariableGet("gv_newSequenceNotAllowed")>0.5;
		GlobalVariableSet("gv_newSequenceNotAllowed",-1);
	}
	// RESET SEQUENCE /////////////////////////////////////
...
if ((int)GlobalVariableGet("gv_createSnapshotForStrategy") == i_stratMagicNumber) {			// only enter loop if 
   ...
   for(int i=0; i<i_namesNumber; i++) { 
	...
	GlobalVariableSet("gv_createSnapshotForStrategy",0);	// reset
}
// START NEW SEQUENCE
...
// INDICATOR BUFFERS ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      ...
      if (m_tradeFlag[i]==true) {
		...
		if ((m_sequence[i][0]<0) || (m_sequence[i][2]<1.5 && m_lastTicketOpenTime[i][0]<1)) {	// no sequence or new sequence opened this bar                      // SF
			...
			if (bollinger_mode==UPPER) {
				f_bollingerBand = MathMax(iBands(m_names[i],timeFrame,(int)m_filter[i][0],bollinger_deviations,0,0,bollinger_mode,1+bollinger_delay),
									iBands(m_names[i],timeFrame,(int)m_filter[i][0],bollinger_deviations,0,0,MODE_MAIN,1+bollinger_delay) + f_minBollingerBandRatio*0.5*m_bandsTSAvg[i]*MarketInfo(m_names[i],MODE_POINT));
				f_bollingerBandPrev = MathMax(iBands(m_names[i],timeFrame,(int)m_filter[i][0],bollinger_deviations,0,0,bollinger_mode,2+bollinger_delay),
									iBands(m_names[i],timeFrame,(int)m_filter[i][0],bollinger_deviations,0,0,MODE_MAIN,2+bollinger_delay) + f_minBollingerBandRatio*0.5*m_bandsTSAvg[i]*MarketInfo(m_names[i],MODE_POINT)); 
			}
			else if (bollinger_mode==LOWER) {
				f_bollingerBand = MathMin(iBands(m_names[i],timeFrame,(int)m_filter[i][0],bollinger_deviations,0,0,bollinger_mode,1+bollinger_delay),
									iBands(m_names[i],timeFrame,(int)m_filter[i][0],bollinger_deviations,0,0,MODE_MAIN,1+bollinger_delay) - f_minBollingerBandRatio*0.5*m_bandsTSAvg[i]*MarketInfo(m_names[i],MODE_POINT));
          ...
          if (temp_T[0] < 0 || temp_T[1] < 0) {
            if (m_sequence[i][0]<0) {         // either new sequence or manually set (-1) mid-sequence, so update the pips
               ...
               // confirm new sequence can be opened for product
			      ...
			      m_productOutOfHours[i] = ((f_timeInHours>m_nonTradingWindows[i][0])&&(f_timeInHours<m_nonTradingWindows[i][1])) || ((f_timeInHours>m_nonTradingWindows[i][2])&&(f_timeInHours<m_nonTradingWindows[i][3]));
            }
            else {
...
for(int i=0; i<i_namesNumber; i++) {
   if (m_tradeFlag[i]==true && m_signal[i]!=0) {
      for(int k=0; k<m_tradesNumber[i]; k++) {
         if (m_signal[i]>0 && m_positionDirection[i][k]<0 && m_lastTicketOpenTime[i][0]!=0) { m_close[i][k]=true; }
         else if (m_signal[i]>1 && m_positionDirection[i][k]==0 && m_lastTicketOpenTime[i][0]!=0) { m_open[i][k]=1; }
         else if (m_signal[i]<0 && m_positionDirection[i][k]>0 && m_lastTicketOpenTime[i][0]!=0) { m_close[i][k]=true; }
         else if (m_signal[i]<-1 && m_positionDirection[i][k]==0 && m_lastTicketOpenTime[i][0]!=0) { m_open[i][k]=-1; }
         else { }// do nothing
      }   
   }
...
// CLOSING ORDERS //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 // OPENING ORDERS //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   if (m_tradeFlag[i]==true) {
      ...
      if (!(b_newSequenceNotAllowed && m_sequence[i][0]<0) && !(m_newSequenceNotAllowed[i] && m_sequence[i][0]<0) && m_productOutOfHours[i]==false) {   
         for(int k=0; k<m_tradesNumber[i]; k++) {
			...
			if (m_open[i][k]>0 || m_open[i][k]<0) {
				...
				// Lots per trade
				f_lotUnsplit = MathMax(m_profitInAccCcy[i],MathAbs(m_loss[i])/m_sequence[i][3]) / m_accountCcyFactors[i] / m_bollingerDeviationInPips[i];
				if (f_lotUnsplit>m_lotMax[i]) { Alert("The lots for ",m_names[i]," are above the permissible amount and will be split."); }
				if (f_lotUnsplit>TRADESPERNAMEMAX * m_lotMax[i]) { Alert("The lots for ",m_names[i]," are ",TRADESPERNAMEMAX," times above the permissible amount and CANNOT be split."); }
				m_lots[i][k] = f_lotUnsplit / m_tradesNumber[i];
				m_lots[i][k] = NormalizeDouble(MathMin(m_lotMax[i],MathMax(m_lotMin[i],m_lots[i][k])),m_lotDigits[i]);  // floor and normalize
			}
      ...
      if (h!=INVALID_HANDLE) {
         for(int i=0; i<i_namesNumber; i++) { 
            FileWrite(h,m_names[i],",",
						...
						NormalizeDouble(m_slowFilterGap[i],5)); 
         } 
         FileClose(h); 
      }
