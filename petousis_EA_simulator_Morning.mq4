...
// INDICATOR BUFFERS ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   //if (TimeMinute(TimeCurrent()) != i_minuteLastCalced) {      // repeat every minute not at every tick - EA is for large timeframes
	...
	if (Hour()==m_names[i,xxx] && Minutes()==m_names[i,xxx]) {
		 // use format 3:23,6:50 given as 3,23,6,50
		 i_SRInMin = (StrToInteger(m_names[i,xxx]) - StrToInteger(m_names[i,xxx])*60 + StrToInteger(m_names[i,xxx]) - StrToInteger(m_names[i,xxx];
		 f_low = 100000;
		 f_high = 0;
		 f_average = 0;
		 for (int ishift=0; ishift<i_SRInHours; ishift++) {
		...
		m_takeProfit[i,1] = NormalizeDouble(m_openPrice[i,1] - f_SR,MODE_DIGITS);
		 ...
		 if (Hour()==m_names[i,xxx] && Minutes()==m_names[i,xxx] && m_state[i,0]==0 && m_state[i,1]==0) {		// should be the starting point -- open two pending orders
