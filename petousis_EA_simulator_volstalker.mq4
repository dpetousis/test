...
// VARIABLE DECLARATIONS /////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   double
   temp_vwap=-1.0,
   SL,TP,BID,ASK,f_fastFilterPrev=0.0,f_central=0.0,f_profit=0.0,
      ...
      if (m_tradeFlag[i]==true) {
			...
			if (res) {
				if (OrderCloseTime()>0) {			// if closed
					if (readTradeComment(m_ticket[i],m_names[i],temp_sequence)) {
						if (temp_sequence[1]+temp_sequence[5]<0) {	//ie trade sequence closed negative 
							m_sequence[i][0] = -1;
							m_sequence[i][1] = 0;
							m_sequence[i][2] = 0; 
   ...
   if (m_tradeFlag[i]==true) {
      if (m_isPositionPending[i]==true && m_signal[i]==0) {     // if pending order exists -> modify pending order
      		...
      		if (m_positionDirection[i]==1) { 
      			...
      			SL=NormalizeDouble(ASK - (f_percWarp-f_percMargin)*m_bollingerDeviationInPips[i]*MarketInfo(m_names[i],MODE_POINT),(int)MarketInfo(m_names[i],MODE_DIGITS));     // Calculating SL of opened
               TP=NormalizeDouble(ASK + m_bollingerDeviationInPips[i]*MarketInfo(m_names[i],MODE_POINT),(int)MarketInfo(m_names[i],MODE_DIGITS));   // Calculating TP of opened
         		...
         		SL=NormalizeDouble(BID + (f_percWarp-f_percMargin)*m_bollingerDeviationInPips[i]*MarketInfo(m_names[i],MODE_POINT),(int)MarketInfo(m_names[i],MODE_DIGITS));     // Calculating SL of opened
           ...
           {                                       // criterion for opening Buy
            ...
            if (m_isPositionPending[i]==false && m_isPositionOpen[i]==false) {       // if no position and no pending -> send pending order
               ...
               /**if (-m_sequence[i][1]<f_creditPenaltyThreshold && f_creditBalance>0) { // apply penalty
                  ...
                  m_creditAmount[i] = (MathFloor(-m_sequence[i][1]/m_profitInUSD[i]))*m_profitInUSD[i];
		            ...
		            b_appliedCredit = true; } 
	       	      ...
	       	      f_profit = m_sequence[i][1]; 
         		  b_appliedCredit = false;
         		  b_appliedPenalty = false;
	            //}
               if (f_profit>m_profitInUSD[i]*f_percWarp) {
      	    	   m_lots[i] = NormalizeDouble(MathMax(m_lotMin[i],(f_profit/f_percWarp) / m_accountCcyFactors[i] / m_bollingerDeviationInPips[i]),m_lotDigits[i]);
         	    }
         	    else {
               ...
               s_comment = StringConcatenate(IntegerToString(m_myMagicNumber[i]),"_",DoubleToStr(temp_vwap,(int)MarketInfo(m_names[i],MODE_DIGITS)),s_adjFlag,"_",DoubleToStr(f_profit,2),"_",DoubleToStr(m_sequence[i][2]+1,0));
               ...
               if (ticket < 0)  {                    
                  ...
                  Alert("Profit: ",f_profit,". SLinUSD: ",m_profitInUSD[i],". Factor: ",m_accountCcyFactors[i],". Pips: ",m_bollingerDeviationInPips[i]);
               }
               else {			// Success :) 
